...
export default class Scanray {
  ...
  static activateMonitor(options: scanrayOptions): void {
    ...
    if (options?.blockKeyboardEventsDuringScan)
      document.addEventListener(
        'keydown',
        Scanray._trapKeyboardEventsDuringScan
      );

    // prevent alt-key keyboard events from creating chars in browser window
    if (options?.blockAltKeyEvents)
      document.addEventListener('keydown', Scanray._trapAltKeyListener);

    // prevent special Ctrl-key sequences from triggering browser controls (as known to occur in AAMVA barcodes)
    if (options?.blockBadKeyboardShortcutEvents)
      document.addEventListener('keydown', Scanray._trapCtrlKeyListener);
  ...
  static onScan(scanData: string, returnData?: boolean): any {
    ...
    if (
      ...
      if (options.enabledLogging)
        console.log(`activeElement.value: [${formValue}]`);
      if (scanValue.length === 0 || formValue.length === 0) return;
      longestCommonSubstring([formValue, scanValue]).forEach(
        (overlappingStringToRemove: string) => {
          // remove text from field likely to have come from scanner => hence a min-length check
          if (
            overlappingStringToRemove.length > minLen &&
            formValue.includes(overlappingStringToRemove)
          )
            activeElement.value = formValue.replace(
  ...
  static keyCodeMapper(e: KeyboardEvent): string {
    ...
    if (onScan.isScanInProgressFor(document)) {
      ...
      if (
        Scanray.accumulatedScanData().length === 0 &&
        Scanray.isPrefix(altKey || e.key)
      ) {
  ...
  static accumulatedScanData(): string {
    return (
      (document as any).scannerDetectionData?.vars?.accumulatedString?.[0] || ''
    );
  }

  static prefixesExpected(): number[] {
    return (
      (document as any)?.scannerDetectionData?.options?.prefixKeyCodes || []
    );
  }
  ...
  static currentScanHasPrefix(): boolean {
    ...
    return (
      Scanray.isPrefix(firstChar) || Scanray.isPrefix(Scanray.currentPrefix)
    );
  }
  ...
  static canBlockKeyboardEvents(): boolean {
    ...
    return (
      onScan.isScanInProgressFor(document) && Scanray.currentScanHasPrefix()
    );
  }
  ...
  static _trapCtrlKeyListener(e: KeyboardEvent): void {
    ...
    if (
      ...
      if (Scanray.options.enabledLogging)
        console.log(
          `Blocked control sequence: [${JSON.stringify({
            code: e.code,
            ctrlKey: e.ctrlKey,
            key: e.key,
          })}]`
