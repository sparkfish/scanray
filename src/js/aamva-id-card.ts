...
// 256 character table known as ASCII/ISO 8859-1 as the character set table when generating Hi-Density symbols and for efficiency shall use the 128 character subset text compaction table.
...
export default class AamvaIdCard {
  ...
  constructor(data: string, delim: string = '|~|') {
    // high-level parsing into card parts and details from header
    this._data = data.trim()
    // TODO: WHAT THE MIN LEN SHOULD THIS CONDITION BE ????????
    const minValidLength = 10
    const headerDelimsLength = delim.length * 2
    const ansiPos = 1 + headerDelimsLength
    if (
      ...
      // split out segmented data elements
      const firstSegmentOffset = (Number.parseInt(this.getSlice(headerInfo, 19, 23)) || 0) - 4
      this._cardParts = this.getSlice(headerInfo, firstSegmentOffset)?.split(delim) || []
    }
  }
  ...
  public get middleName(): string {
    ...
    return (
      (this.getSegmentValue('DBQ', undefined) || this.getSegmentValue('DAD', undefined) || '') +
      (wasTruncted ? ']' : '')
    )
  }
